...
<template>
  <div class="flex flex-col mb-[1.6rem] relative" :class="width">
    <label
      :for="inputName"
      class="text-[1.6rem] text-[#ffffff] mb-[0.8rem] md:mb-[0px]"
      >{{ label }}</label
    >
    <div class="relative">
      <Field
      ...
      <password-hidden
        ...
        v-if="inputType == 'password' && !noState && !validState"
        class="absolute top-1/2 right-0 -translate-x-3/4 -translate-y-1/2"
      ></password-hidden>
      <profileinvalid-icon
        v-if="noState"
    ...
    <ErrorMessage
      :id="`${inputName}Error`"
      style="color: #ec9524; font-size: 14px; margin-top: 4px"
      :name="inputName"
    />
  </div>
</template>
